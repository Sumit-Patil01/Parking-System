...
def draw_polylines(frame, area, index, is_occupied):
...
def process_frame(frame, areas):
    ...
    for i, area in enumerate(areas):
        draw_polylines(frame, area, i + 1, occupied[i])
    ...
    cv2.putText(frame, f"Parked Spaces: {len(areas) - len(empty_spaces)}", (10, 30 * (len(areas) + 3)), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2)

    return frame, len(empty_spaces), len(areas) - len(empty_spaces), empty_spaces
...
while True:
    ...
    frame, empty_spaces, parked_spaces, empty_spaces_list = process_frame(frame, areas)
